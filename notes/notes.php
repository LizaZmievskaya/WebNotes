...
<body>
    <div id="notes">
        <div id="title">
            ...
            <input type="button" class="file" value="Применить" name="accept">
        </div>
        <div id="scroll">
            <div id="text">Введите название и текст, затем нажмите кнопку "Добавить" для добавления заметки.</div>
            <?php while($row=mysql_fetch_assoc($query)){ ?>
            <div style="background-color:<?=$row['color']?>" data-id="<?=$row['id_note']?>" class="left_notes"><div><?=$row['name']?></div><div><?=$row['content']?></div></div><?php } ?>
        </div>
    </div>
    ...
    <div id="content">        
        <form action="/notes/save.php" method="post" enctype='multipart/form-data'>
            ...
            <div id="btns">
                <input name="attach" class="file" type="file">
                <input name="save" class="button" type="submit" value="Добавить">
